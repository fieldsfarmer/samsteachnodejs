...
def TweetSearch(keywords, output_file, interval):
	...
	print "start searching tweets"
	try:
		results = twitter.search(q=keywords, result_type = 'recent', count = '100', since_id = since_id)  #keyword -> hashtag
	except Exception:
		output.write(max_id + '\n')
		output.close()
		return
...
class MyStreamer(twython.TwythonStreamer):
	...
	def on_success(self, data):
		...
		output = self.output
            	...
            	#write trend category this tweet belongs to, connected by ','
		text = tweet.get('text')
		trend_text = ''
		if text is not None:
		...
		if tweet.get('lang') == 'en' and tweet.get('entities').get('hashtags') and not trend_text:
			print 'catch a valid tweet!'
			#write trend category
			output.write('trend: ' + trend_text + '\t')	
			...
			coordinates = tweet.get('coordinates')
			if coordinates is not None:
				print 'this tweet has geolocation'
				output.write(str(coordinates.get('coordinates')[0]) + ',' + str(coordinates.get('coordinates')[1]))
			else:
				output.write('NULL')
			output.write('\t')
...
def TweetStreamingThread(keywords, output_file):
	while True:
		try:
			...
			query = ("SELECT trend FROM trends GROUP by trend")
			query_cursor.execute(query)
			for (trend,) in query_cursor:
				all_trends.append(trend)
			print all_trends
			stream.statuses.filter(track=all_trends) 
			#if exception, filter again from scratch, some repetative data
		except Exception:
			continue
